...
class ImplementAction(BaseAction):
    ...
    def execute(self, issue, jira_client, github_client, claude_executor) -> str:
        ...
        branch_name = generate_branch_name(BRANCH_PREFIX_FEATURE, issue_key)
        ...
        try:
            ...
            # Compose PR content
            pr_title = f"{COMMIT_PREFIX_FEAT}{summary} [{issue_key}]"
            pr_body = (
                f"## Summary\n\n"
            ...
            if not pr_validation.is_valid:
                ...
                jira_client.add_comment(issue_key, comment)
                jira_client.remove_label(issue_key, self.label)
                return f"Implementation PR blocked for {issue_key}: {pr_validation.failure_reason}"
